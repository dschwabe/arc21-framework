...
## Stage 2 — Verify framework facts  · framework repo, scratch copies  · ⛔ REVIEW GATE
Goal: establish how `sync.py` and the loader behave today, so Stage 3 is designed right.
Doc: `sync-verification-runbook.md` (framework repo `docs/`), questions Q1–Q7.
Steps: `cp -R` both repos to `/tmp`; run the runbook's checks; fill in its report-back template.
**Stop here — review the report before writing any framework code.** It decides whether changes 2 and 5 are additive or need a refactor (especially Q7: does the loader hard-assume a single concept source).
Independent of Stage 1; can run in parallel (touches nothing live).

## Stage 3 — Implement framework extensibility  · framework repo  · ⛔ REVIEW GATE at end
Goal: the generic mechanisms that let HyperTorah's columns and the future registry work, with **no domain content** entering the framework.
Doc: `framework-extensibility-design.md` (framework repo `docs/`), informed by Stage 2's report.
Order:
1. **Change 1** — preserve unknown columns → `extra` on concept / relation / relationType.
2. **Change 2** — multi-source concept merge (registry mechanism), **backward-compatible by construction**: single source = the N=1 case of one union path; keyed on `sefariaRef`; site fields override registry per-key; site composites add nodes.
3. **Change 3** — site-owned skin resolution + reserved skin namespace (generalise the hero-diagram dispatcher).
4. **Change 5** — `sync.py` ownership manifest + reserved namespaces (`js/site/**`, site skins, hook path); report collisions rather than clobber.
5. **Change 4** — specify the `afterParse(graph, rawSheets, siteConfig)` hook signature; **do not implement** (no consumer yet).
Then bump `ARC21_VERSION` across its full checklist (`js/version.js`).
Gate: an existing registry-less site (one of your other two) still loads **unchanged** after these changes + a sync. **Stop and confirm this before Stage 4** — this is the backward-compatibility guarantee, tested rather than assumed.
