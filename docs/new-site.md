...
site.css            palette
index.html          public shell (title, meta)
mgmt.html           management shell (generated by build.py)
...
Every ARC21 site has two HTML entry points that share a **single source of truth**:

| File | URL | Purpose |
|------|-----|---------|
| `index.html` | `https://your-site.tld/` | **Public** — auto-loads the XLSX, management UI hidden |
| `mgmt.html` | `https://your-site.tld/mgmt.html` | **Management** — full file-picker, narrative import, loader card |

Firebase Hosting serves static files before applying SPA rewrites, so no
rewrite rule is needed for `mgmt.html`.

### How it works

`index.html` is the **only file you edit**. Management-only elements are marked
with `class="mgmt-only"` and a single CSS rule in `default.css` hides them in
public mode:

```css
html:not([data-mgmt]) .mgmt-only { display: none !important; }
```

`mgmt.html` is **auto-generated by `build.py`** — it is identical to
`index.html` except for two differences:

1. `<html data-mgmt>` instead of `<html>` — activates management elements
2. `<title>` gets `— Gestão` appended

Never edit `mgmt.html` directly. Run `build.py` after changing `index.html`.

### Management-only elements (class="mgmt-only")

- `#changeSourceBtn` — "Trocar arquivo" topbar button and its hidden file input
- `#narrativeImportDialog` — narrative import dialog
- `#heroChangeSourceBtn` — hero "Trocar arquivo de dados" button
- `#heroLoaderCard` — hero manual file-loader card
- `#dataStatus` — data status div

### Workflow for HTML changes

```bash
# 1. Edit index.html only
# 2. Regenerate mgmt.html
python3 build.py

# 3. Check both files, commit together
git status data/ assets/
git add index.html mgmt.html
...
`mgmt.html` inherits the site title from `index.html` with `— Gestão`
appended. To customise the management title, add a post-processing step in
`build.py` or edit the generated `mgmt.html` once after first creation.
