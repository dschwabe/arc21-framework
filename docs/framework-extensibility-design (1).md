...
- **Verified (Stage 2):** additive over today's single-source parse — wrap the existing per-sheet build, add a union step, `js/graph/builder.js` untouched. **Depends on Change 1** — `extra.sefariaRef` only exists once Change 1 ships, so Change 2 cannot be built or tested first. Merge key is `extra.sefariaRef` with `conceptID` fallback. Add a `merge_semantics_version` field to `framework.lock` (additive; absent = no merge).
...
### 3. Site-owned rendering via skins — verified: no framework code
Stage 2 confirmed this already works by construction: the loader imports `skins/<id>/<id>.js` *relative to itself*, resolving to the site root regardless of who placed the folder there, and the additive-merge sync never touches a path the framework doesn't ship. A site ships `concept-hypertorah`, lists it in its own `skins/index.json` (site-owned), and it loads and survives every sync. **Deliverable is documentation only:** a naming convention that site skins use a project-specific id (never a generic one a future framework skin might claim). Document with Change 5 — same collision-risk class.
...
### 5. sync.py — collision detection (verified: reservation is already free)
Stage 2 confirmed the sync is additive-merge: reserved namespaces (`js/site/**`, site skin folders) already survive untouched, so no reservation work is needed. The real gap is the opposite direction — if the framework ever ships a file at a path a site already occupies, `shutil.copy2` overwrites it silently. Change 5 is therefore a **pre-copy collision check** that reports rather than clobbers, not a namespace mechanism. **Rollout caveat:** `sync.py` is itself `SITE_OWNED`, so this logic does not reach existing sites via a normal sync — each needs `example/sync.py` re-copied once.
...
## Verified (Stage 2)
`sync-verification-report.md` answered Q1–Q7. Key results: the sync is **additive-merge** (no deletes; site files survive); the parser is **`js/parse/workbook.js`** (architecture.md's `js/graph/parser.js` is stale and should be removed); unknown columns are dropped at three allowlist points (Change 1's insertion sites); concept assembly is single-source, keyed by `conceptID`, with wholesale replace — so the N-source union is **additive** (N=1 = identity, no `builder.js` refactor); and site skins plus reserved namespaces already work by construction (Change 3 and most of Change 5 need no code). The descriptions above already reflect these findings.
